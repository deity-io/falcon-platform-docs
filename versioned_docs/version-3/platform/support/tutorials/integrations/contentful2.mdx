...
Notice we are using `<Helmet>` to change the page title. To learn more about this read how [falcon manages meta data via helmet](/storefront/meta-data).
...
`<SwitchDynamicURL>` is used for routing in Falcon Platform, it's worth looking at routing details [here](/storefront/routing).
...
`getFetchUrlPriority` is used to set priority of your API. You might want to alter the priority if you're worried about URLs clashing. This is covered in our [docs](/storefront/routing).
