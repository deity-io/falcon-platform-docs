...
const EnvironmentLayout = ({ environments }) => {
  ...
  return (
    <div className={styles.environments}>
      <span
        className={classnames(styles.environmentCount, environmentCount > 1 ? styles.environmentCountHighlighted : '')}
      >
...
const PaymentPeriodSwitcher = ({ monthly, setMonthly }) => (
  <label htmlFor="payment-switcher" className={styles.switcher}>
    ...
    />
    <span className={classnames(styles.label, monthly ? styles.label__active : null)}>Monthly</span>
    <span className={classnames(styles.switch, !monthly ? styles.switch__active : null)} />
    <span className={classnames(styles.label, !monthly ? styles.label__active : null)}>
  ...
  <nav className={styles.tabs}>
    {PricePlans.map(plan => (
      <button
        key={`${plan.name}-tab`}
        type="button"
        className={classnames(styles.tab, activePlan === plan.name ? styles.activeTab : null)}
    ...
    <tbody className={styles.table__body}>
      <tr className={styles.table__row}>
        ...
        {PricePlans.map(plan => (
          ...
          >
            <div className={classnames(styles.cellCurve, styles.cellCurve__top)} />
      ...
      <tr className={styles.table__row}>
        <td className={classnames(styles.table__head, styles.table__cellFoot)}>&nbsp;</td>
        {PricePlans.map(plan => (
          <td
            key={`${plan.name}-foot`}
            className={classnames(
          ...
          >
            <div className={classnames(styles.cellCurve, styles.cellCurve__bottom)} />
          </td>
        ))}
      </tr>
    </tbody>
