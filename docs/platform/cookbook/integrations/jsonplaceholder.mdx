...
For detailed information about integrating with new APIs please [read these docs](/docs/open-source/falcon-server/customizing-extension-api).
...
**n.b.** In this tutorial we will be adding everything directly into our repositiry `client/src` and `server/src`. If you plan to reuse your functionality it's highly recommended you add it to your own `NPM` packages.
...
module.exports = class JsonPlaceholderApi extends ApiDataSource {
  ...
  // Get todo list with pagination passed to it
  getTodoList(pagination) {
    const query = {};
  ...
  async todoList(_, { pagination }) {
    const todos = await this.getTodoList(pagination);
...
Again, notice the naming convention. `[package]-[data_type]-extension`. Unlike the API package, we name this based on the data type not the source. This is because we may at any point swap out the source but use the same extension.
...
Because we aren't adding our code to `NPM` pacakges we are going to add the code to our `client/src/components` directory.
...
<img src="/img/docs/platform/todolist.png" alt="todo list component" width="300" />
