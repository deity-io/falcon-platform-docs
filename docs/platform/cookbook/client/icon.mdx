...
To get more information on how icons work in Falcon Client read this [doc](/docs/platform/client/theming/icons).
...
We now need to pass the `icons` to our theme. In your `theme.js` file you can pass it to the object in `createTheme`.
...
**One 'gotcha' is that some SVGs don't cammel case `viewBox` e.g. `viewbox`. This will cause issues so it's best to change it to `viewBox`.**

Add your svg file to your assets directory (`client/src/assets/new-icon.svg`) and import it in the icon component (`client/src/styling/icons.js`).
...
`<Icon>` is themable so you can pass css like any other component.
