...
In this tutorials we will look at **dynamic routing** using the `extension` from [part 1](/docs/platform/cookbook/integrations/contentful).

## Overview

Using the `course` data from [part 1](/docs/platform/cookbook/integrations/contentful) we will make routes like `https://your-site.com/course/course-name` render content for that course.
...
Notice we are using `<Helmet>` to change the page title. To learn more about this read how [falcon manages meta data via helmet](/docs/platform/client/meta-data).
...
`<SwitchDynamicURL>` is used for routing in Falcon Platform, it's worth looking at routing details [here](/docs/platform/client/routing).
...
`getFetchUrlPriority` is used to set priority of your API. You might want to alter the priority if you're worried about URLs clashing. This is covered in our [docs](/docs/platform/client/routing).
