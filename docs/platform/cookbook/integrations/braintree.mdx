...
To get this up and running we will do this in our `server/config/local.json`. For production config you should read [these docs](/docs/platform/client/configuration).
...
Each payment provider should include 2 methods. `initialize` when the providier is initialised and `validate` when the payment is submitted.
...
module.exports = class BraintreePayment {
  ...
  // payload contains information about the paymemnt, e.g. total, currency, customer ID
  async initialize(payload) {
    return {
...
We will install this pacakge `client` side. It's does a lot of the heavy lifting for rendering a Braintree form with **hosted fields**.
...
```bash
yarn add react-braintree-fields
```
...
If you visit the checkout now you should now see Braintree as an option and should also see the form appearing as expected.
...
This package allows us to easilly make well formatted GraphQL requests to 3rd party clients.
