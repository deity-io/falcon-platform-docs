...
### API root
You can make use of the by Commercetools [TypeScript SDK](https://commercetools.github.io/nodejs/sdk) through the `apiRoot`. Our `ctpClient` provides two async methods to fetch a pre-configured `apiRoot`, in which all required project configuration, authorization and session refreshing are handled:
- **`integrationApiRoot()`**
To make requests in the name of the [commercetools project](https://docs.commercetools.com/merchant-center/projects), use the `integrationApiRoot`. This is for all requests where a signed-in user is not required.
- **`customerApiRoot()`**
To make requests in name of the customer, use `customerApiRoot`.
