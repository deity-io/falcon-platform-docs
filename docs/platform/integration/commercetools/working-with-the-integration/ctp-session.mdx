...
- `ctpSession` provides functions and setter methods to manipulate the session

### Authentication

The `customerToken` and `customerId` will be set whenever a customer is authenticated. When an unauthenticated customer (guest) opens a cart we will create an anonymous `customerToken` and store it in session. In this case the customerId will remain empty.
