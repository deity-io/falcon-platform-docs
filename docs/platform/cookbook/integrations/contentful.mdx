...
[This example](json-placeholder) covers this section in more detail.
...
These are registered in your `server/config/default.json`. Remeber the api passed to the extension must match the name defined in the api, in this case `contentful`.
...
To use this tool it must be running on post `4000` (`http://localhost/4000/graphql`).
...
As before, this section is largely covered in [this example](json-placeholder).
...
In [step 3](#3-adding-your-credentials-locally) we added our API keys to our `config/local.json` file. This isn't commited so won't work anywhere other than locally.

To add use this config on a remote environment we will want to use a `server/config/custom-environment-variables.json` file.
...
That's it, you now have a **basic** integration with Contentful. Contentful is a really powerful tool and the integration possibilites are endless but the principle should always remain the same.
