...
severity: indicates the severity classification of the entry. Default severity values - from lowest to highest - are trace, info, debug, warning, error and critical.

app: indicates from which application the log entry was produced — either falcon-server or falcon-client.

name: indicates the module which produced the log. For composer logs, this is typically the name of the platform module

durationMs: the amount of milliseconds it took to resolve an outgoing request

msg: the log message

timestamp: the timestamp of the log in GMT+2
