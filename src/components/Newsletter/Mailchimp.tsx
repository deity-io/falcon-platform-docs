...
const ResponseMessage = ({ response }) => (
  <p style={{ color: response.result === 'error' ? 'red' : 'green' }}>{response.message}</p>
);
      ...
      {({ isSubmitting }) => (
        <Form>
          <div className={styles.inputs}>
            ...
            />
            <button type="submit" disabled={isSubmitting} className={classNames(styles.button, 'button')}>
              Sign Up
            </button>
          </div>
          {response ? (
            ...
            <ErrorMessage name="email" component="p" className={styles.errorEmail} />
          )}
        </Form>
      )}
    </Formik>
